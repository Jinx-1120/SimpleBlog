...
import { HttpForbiddenError, HttpBadRequestError } from '@app/errors'
...
export const QueryParams = createParamDecorator((customConfig: TTransformConfig[], request: any): IQueryParamsResult => {
  ...
  ].map(item => (item !== null && item !== undefined) ? Number(item) : item)
  // 参数提取验证规则
  ...
  const validates: IValidateError[] = [
    ...
    },
    {
      ...
      setValue() {
        ...
        per_page ? options.limit = per_page : config.app.limit
      }
    },
    {
